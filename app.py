...
def analiz_sayfasi():
    ...
    if st.session_state.get("cache_cleared", False):
        st.success("✅ Cache başarıyla temizlendi! Sayfa güncel verilerle yenilendi.")
        # Bildirimi gösterdikten sonra flag'i temizle (sadece bir kez göster)
...
def rapor_sayfasi():
    """Basitleştirilmiş Rapor Sayfası - Sadece son nakil analizi sonuçları"""
    st.markdown("<h1 class='main-header'>📊 Nakil Analizi Sonuçları</h1>", unsafe_allow_html=True)
    ...
    # Sadece son raporları al
    reports_all = get_existing_reports()
    if not reports_all:
        st.warning("⚠️ Henüz oluşturulmuş rapor bulunamadı.")
        st.info("📝 Rapor oluşturmak için **Nakil Analizi** sayfasından bir Excel dosyası yükleyin.")
        return

    # En son tarihteki raporu otomatik seç
    dates = sorted({r["tarih"] for r in reports_all}, reverse=True)
    latest_date = dates[0]
    latest_reports = [r for r in reports_all if r["tarih"] == latest_date]
    
    # En son oluşturulan raporu seç (klasör adına göre en yeni)
    selected_report = max(latest_reports, key=lambda x: x["folder"])
    
    # Rapor bilgilerini göster
    st.info(f"📅 **En Son Analiz Tarihi:** {selected_report['tarih']}")
    if selected_report.get('excel'):
        st.info(f"📄 **İşlenen Excel Dosyası:** {selected_report['excel']}")
    
    reports = [selected_report]  # Sadece son raporu göster
    
    # Direkt son raporu göster
    selected = reports[0]
