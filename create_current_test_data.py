...
def create_current_test_data():
    ...
    # Tarihleri üç gruba ayır:
    # %40 dün (2025-09-16), %40 bugün (2025-09-17), %20 önceki günler
    dun_vaka_sayisi = int(n_rows * 0.4)
    bugun_vaka_sayisi = int(n_rows * 0.4)
    onceki_vaka_sayisi = n_rows - dun_vaka_sayisi - bugun_vaka_sayisi
    
    tarihler = []
    
    # Dünkü vakalar (2025-09-16)
    for _ in range(dun_vaka_sayisi):
        tarih = dun_baslangic + timedelta(
            hours=np.random.randint(24),
            minutes=np.random.randint(60)
        )
        tarihler.append(tarih)
    
    # Bugünkü vakalar (2025-09-17)
    for _ in range(bugun_vaka_sayisi):
        tarih = bugun_baslangic + timedelta(
            hours=np.random.randint(24),
            minutes=np.random.randint(60)
        )
        tarihler.append(tarih)
    
    # Önceki günlerin vakaları
    for _ in range(onceki_vaka_sayisi):
        tarih = dun_baslangic - timedelta(
    ...
    print(f"Güncel tarihli test verisi oluşturuldu: {output_path}")
    print(f"Veri özeti:")
    print(f"- Toplam vaka: {len(df)}")
    
    # Tarih dağılımı
    tarihler_dt = pd.to_datetime(df["oluşturma tarihi"])
    dun_count = ((tarihler_dt >= dun_baslangic) & (tarihler_dt < bugun_baslangic)).sum()
    bugun_count = (tarihler_dt >= bugun_baslangic).sum()
    onceki_count = len(df) - dun_count - bugun_count
    
    print(f"- Dün (2025-09-16): {dun_count} vaka")
    print(f"- Bugün (2025-09-17): {bugun_count} vaka")  
    print(f"- Önceki günler: {onceki_count} vaka")
