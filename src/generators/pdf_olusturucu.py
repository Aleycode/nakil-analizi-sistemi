...
class PDFOlusturucu:
    ...
    def pdf_olustur(self, grafik_dizini: Path, gun_tarihi: str, analiz_verisi: Dict, unique_id: str = None) -> str:
        """
        ...
        try:
            ...
            # PDF dosya adına unique_id ekle
            pdf_dosya_sablonu = self.config.get(
                "pdf_dosya_adi", "nakil_analiz_raporu_{tarih}.pdf"
            )
            
            if unique_id:
                # Şablon içinde {tarih} varsa, {tarih}_{unique_id} olarak değiştir
                pdf_dosya_adi = pdf_dosya_sablonu.replace("{tarih}", f"{gun_tarihi}_{unique_id}")
            else:
                pdf_dosya_adi = pdf_dosya_sablonu.format(tarih=gun_tarihi)
