...
def configure_page():
    ...
    st.set_page_config(
        page_title="Nakil An                # PDF raporu - Ana bölüm olarak göster, tüm rapor içeriğini tek bir PDF'de göster
                pdf_path = DATA_REPORTS_DIR / selected_date / f"nakil_analiz_raporu_{selected_date}.pdf"
                
                # Başka bir PDF raporu da kontrol et (günlük işlem sonrasında oluşan)
                if not os.path.exists(pdf_path):
                    processed_dir = ROOT_DIR / "data" / "processed"
                    try:
                        tarih_obj = datetime.strptime(selected_date, "%Y-%m-%d")
                        folder_date = tarih_obj.strftime("%Y%m%d")
                        alt_pdf_path = processed_dir / f"günlük_{folder_date}" / f"nakil_analiz_raporu_{selected_date}.pdf"
                        
                        if os.path.exists(alt_pdf_path):
                            pdf_path = alt_pdf_path
                    except Exception as e:
                        st.error(f"Alternatif PDF aranırken hata: {e}")
                
                # PDF dosyasını göster
                if os.path.exists(pdf_path):
                    st.markdown("### 📄 Tüm İstatistik ve Grafikler - PDF Raporu", help="Bu PDF raporu, tüm istatistikleri ve grafikleri içerir.")
