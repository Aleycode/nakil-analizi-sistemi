...
def file_uploader_section():
    ...
    st.markdown("<h2 class='sub-header'>Excel Dosyası Yükle</h2>", unsafe_allow_html=True)
    
    uploaded_file = st.file_uploader("Nakil Vaka Excel Dosyası Seçin", type=["xls", "xlsx"])
...
def existing_files_section():
    ...
    st.markdown("<h2 class='sub-header'>Mevcut Excel Dosyaları</h2>", unsafe_allow_html=True)
    
    excel_files = get_raw_files()
    
    if not excel_files:
        st.info("Henüz yüklenmiş Excel dosyası bulunmuyor.")
        return
    
    selected_file = st.selectbox("İşlemek için dosya seçin", excel_files)
    
    if selected_file and st.button("Seçilen Dosyayı İşle"):
