...
def gunluk_islem_yap(excel_dosya: str) -> None:
    ...
    try:
        ...
        sonuc = isleyici.ham_veriyi_isle(excel_dosya)
...
def gunluk_nakil_analizi_yap(
    ...
    try:
        if gun_tarihi is None:
            if gun_tipi == "bugun":
                # Bugün (normal) - bugünün tarihi
                gun_tarihi = datetime.now().strftime("%Y-%m-%d")
            else:
                # Dün (normal) - bugünden 1 gün çıkar
                gun_tarihi = datetime.now() - timedelta(days=1)
                gun_tarihi = gun_tarihi.strftime("%Y-%m-%d")
        else:
            # Kullanıcı tarih verdi, gun_tipi'ne göre ayarla
            if gun_tipi == "bugun":
                # Verilen tarih aynen kullanılır
                gun_tarihi = gun_tarihi
            else:
                # Dün için, verilen tarihten 1 gün çıkar
                verilen_tarih = datetime.strptime(gun_tarihi, "%Y-%m-%d")
                gun_tarihi = (verilen_tarih - timedelta(days=1)).strftime("%Y-%m-%d")

        # Zaman aralığını hesapla ve göster
        gun_datetime = datetime.strptime(gun_tarihi, "%Y-%m-%d")
        if gun_tipi == "bugun":
            # Bugün için: bugün 08:00 - yarın 08:00
            baslangic_tarihi = gun_tarihi  # bugün
            bitis_tarihi = (gun_datetime + timedelta(days=1)).strftime("%Y-%m-%d")  # yarın
        else:
            # Dün için: dün 08:00 - bugün 08:00  
            baslangic_tarihi = gun_tarihi  # dün tarihi
            bitis_tarihi = (gun_datetime + timedelta(days=1)).strftime("%Y-%m-%d")  # bugün

        logger.info(f"Günlük nakil analizi başlatılıyor: {gun_tarihi} ({gun_tipi})")
        logger.info(f"Zaman aralığı: {baslangic_tarihi} 08:00 - {bitis_tarihi} 08:00")

        analizci = NakilAnalizcisi()
        rapor = analizci.kapsamli_gunluk_analiz(bitis_tarihi, baslangic_tarihi)
...
def main():
    ...
    parser.add_argument(
        "--nakil-analysis", 
        action="store_true", 
        help="Son günün nakil analizini yaparak PDF rapor oluştur"
    )
    ...
    try:
        ...
        elif args.nakil_analysis:
            # En son günün verisiyle analiz yap
            bugun = datetime.now().strftime("%Y-%m-%d")
            gunluk_nakil_analizi_yap(bugun, "dun")
        else:
