...
def gunluk_nakil_analizi_yap(
    ...
    """
    try:
        analiz_tarihi_str = gun_tarihi
        if analiz_tarihi_str is None:
            if gun_tipi == "bugun":
                analiz_tarihi_str = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
            else:
                analiz_tarihi_str = datetime.now().strftime("%Y-%m-%d")
        
        gun_datetime = datetime.strptime(analiz_tarihi_str, "%Y-%m-%d")
        
        # İşlenmiş veri dosyasının yolunu belirle
        # Analiz tipi 'dün' ise dünün tarihini, 'bugün' ise bugünün tarihini kullan
        if gun_tipi == "dun":
            # Dün analizi için, dünün verisi kullanılır.
            # Ancak program akışında veri bugünün tarihiyle işlenir.
            # Bu yüzden analiz dünün aralığını kapsasa da bugünün işlenmiş verisini kullanır.
            gunluk_dizin_str = datetime.now().strftime("%Y%m%d")
        else: # gun_tipi == "bugun"
            # Bugün analizi de aynı şekilde bugünün işlenmiş verisini kullanır.
            gunluk_dizin_str = datetime.now().strftime("%Y%m%d")

        veri_dosyasi_yolu = ISLENMIŞ_VERI_DIZIN / f"günlük_{gunluk_dizin_str}" / "veriler.parquet"
        
        if not veri_dosyasi_yolu.exists():
            logger.error(f"Analiz için veri dosyası bulunamadı: {veri_dosyasi_yolu}")
            print(f"❌ Hata: Analiz için {gunluk_dizin_str} tarihli veri dosyası bulunamadı.")
            return

        baslangic_tarihi = (gun_datetime - timedelta(days=1)).strftime("%Y-%m-%d")

        logger.info(f"Günlük nakil analizi başlatılıyor: {analiz_tarihi_str} ({gun_tipi})")
        logger.info(f"Zaman aralığı: {baslangic_tarihi} 08:00 - {analiz_tarihi_str} 08:00")

        analizci = NakilAnalizcisi()
        rapor = analizci.kapsamli_gunluk_analiz(analiz_tarihi_str, veri_dosyasi=veri_dosyasi_yolu)
