...
def configure_page():
    ...
    # Footer ekle - animasyonlu geçiş (Streamlit state ile)
    import time
    
    # Session state ile timer kontrol et
    if 'footer_start_time' not in st.session_state:
        st.session_state.footer_start_time = time.time()
    
    # Geçen süreyi hesapla
    elapsed_time = time.time() - st.session_state.footer_start_time
    
    # 3 saniye sonra sadece ikonu göster
    if elapsed_time < 3:
        # İlk 3 saniye: tam metin
        st.markdown(
            """
            <div class="footer">
