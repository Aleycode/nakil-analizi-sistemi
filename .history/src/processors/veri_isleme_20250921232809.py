...
class VeriIsleme:
    ...
    def veriyi_oku(self, dosya_yolu: Optional[Path] = None) -> pd.DataFrame:
        """Ana veri dosyasını veya belirtilen bir parquet dosyasını okur."""
        try:
            hedef_dosya = dosya_yolu if dosya_yolu else self.ana_veri_dosya
            if not hedef_dosya.exists():
                raise FileNotFoundError(f"Veri dosyası bulunamadı: {hedef_dosya}")

            df = pd.read_parquet(hedef_dosya)
