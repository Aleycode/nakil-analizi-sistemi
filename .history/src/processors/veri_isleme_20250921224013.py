...
from ..core.config import (
    ISLENMIŞ_VERI_DIZIN,
    HAM_VERI_DIZIN,  # Eklendi
    DESTEKLENEN_FORMATLAR,  # Eklendi
    TARIH_SUTUNLARI,
)
from .data_processor import Veriİşleyici  # Eklendi
...
class VeriIsleme:
    ...
    def veriyi_oku(self) -> pd.DataFrame:
        """
        Ana veri dosyasını okur. Dosya yoksa ham veriden oluşturur.
        """
        try:
            if not self.ana_veri_dosya.exists():
                logger.warning("Ana veri dosyası bulunamadı. Ham veriden otomatik işleme başlatılıyor...")
                
                # Ham veri klasöründeki en son Excel dosyasını bul
                raw_files = []
                for ext in DESTEKLENEN_FORMATLAR:
                    raw_files.extend(list(HAM_VERI_DIZIN.glob(f"*{ext}")))
                
                if not raw_files:
                    raise FileNotFoundError("İşlenecek ham veri dosyası bulunamadı.")

                # En son değiştirilen dosyayı seç
                latest_file = max(raw_files, key=lambda p: p.stat().st_mtime)
                logger.info(f"İşlenecek en son dosya bulundu: {latest_file}")

                # Veri işleyiciyi kullanarak ana veriyi oluştur
                veri_isleyici = Veriİşleyici()
                sonuc = veri_isleyici.gunluk_islem(str(latest_file))
                
                if not sonuc.get("basarili", False) or not self.ana_veri_dosya.exists():
                    raise FileNotFoundError("Otomatik veri işleme sonrası ana veri dosyası oluşturulamadı.")
                
                logger.info("Ana veri dosyası ham veriden başarıyla oluşturuldu.")
    ...
    def gunluk_zaman_araligi_filtrele(
        self, df: pd.DataFrame, gun_tarihi: Optional[str] = None, baslangic_tarihi: Optional[str] = None
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if baslangic_tarihi is None:
                # Eski mantık: dün 08:00 - bugün 08:00
                bugun_08 = pd.to_datetime(f"{gun_tarihi} 08:00:00")
                dun_08 = bugun_08 - timedelta(days=1)
            else:
                # Yeni mantık: başlangıç tarihi - bitiş tarihi  
                dun_08 = pd.to_datetime(f"{baslangic_tarihi} 08:00:00")
                bugun_08 = pd.to_datetime(f"{gun_tarihi} 08:00:00") + timedelta(days=1)
