...
class Veriİşleyici:
    ...
    def veri_temizle(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Tarih sütunu yoksa ekle
            if TARIH_KOLON_ADI.lower() not in df_temiz.columns:
                df_temiz[TARIH_KOLON_ADI.lower()] = datetime.now().date()
    ...
    def ana_veriyi_guncelle(self, yeni_df: pd.DataFrame) -> None:
        ...
        try:
            if self.ana_parquet_dosya.exists():
                ...
                # Tarih sütununu datetime.date tipine dönüştür ve sırala
                if TARIH_KOLON_ADI in birleşik_df.columns:
                    birleşik_df[TARIH_KOLON_ADI] = pd.to_datetime(birleşik_df[TARIH_KOLON_ADI]).dt.date
