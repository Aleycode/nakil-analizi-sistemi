"""Grafik oluşturma için sabit değerler ve yardımcı fonksiyonlar.""""""Grafik oluşturma için sabit değerler ve yardımcı fonksiyonlar.""""""Grafik oluşturma için sabit değerler ve yardımcı fonksiyonlar."""
...
plt.style.use('seaborn')import plotly.graph_objects as goimport plotly.graph_objects as go

plt.rcParams['font.family'] = 'sans-serif'

plt.rcParams['font.sans-serif'] = ['Arial', 'DejaVu Sans']import plotly.io as pioimport plotly.io as pio
...
GRAFIK_RENK_PALETI = [
    ...
    '#3498DB',  # Ana kategori - Mavi# Erişilebilir ve kullanıcı dostu renk paleti (Color-blind friendly)# Temel renk paleti (Color-blind friendly)

    '#F1C40F',  # İkincil kategori - Sarı

    '#2ECC71',  # Olumlu durumlar - YeşilGRAFIK_RENK_PALETI = [GRAFIK_RENK_PALETI = [

    '#E74C3C',  # Uyarılar - Kırmızı

    '#9B59B6',  # Özel durumlar - Mor    '#3498DB',  # Ana kategori - Mavi    '#2E86C1',  # Mavi - Ana kategori

    '#1ABC9C',  # Nötr durumlar - Turkuaz

    '#E67E22',  # Önemli - Turuncu    '#F1C40F',  # İkincil kategori - Sarı    '#F39C12',  # Turuncu - İkincil kategori
...
# Grafik boyutları    '#E74C3C',  # Uyarılar - Kırmızı    '#E74C3C',  # Kırmızı - Uyarılar

VARSAYILAN_GRAFIK_BOYUTU = (10, 8)

BUYUK_GRAFIK_BOYUTU = (12, 10)    '#9B59B6',  # Özel durumlar - Mor    '#8E44AD',  # Mor - Özel durumlar
...
BASLIK_FONT_BOYUTU = 14    '#E67E22',  # Önemli - Turuncu    '#D35400',  # Koyu Turuncu - Önemli

ETIKET_FONT_BOYUTU = 10

LEJANT_FONT_BOYUTU = 9    '#34495E',  # Diğer - Lacivert    '#2C3E50',  # Lacivert - Diğer
...
GRAFIK_STIL = {
    ...
    'axes.edgecolor': '#343A40',# Grafik boyutları# Plotly tema ayarları

    'axes.labelcolor': '#212529',

    'axes.grid': True,VARSAYILAN_GRAFIK_BOYUTU = (10, 8)PLOTLY_TEMA = {

    'grid.alpha': 0.2,

    'text.color': '#212529',BUYUK_GRAFIK_BOYUTU = (12, 10)    'layout': {

    'xtick.color': '#495057',

    'ytick.color': '#495057',KUCUK_GRAFIK_BOYUTU = (8, 6)        'font': {'family': 'Arial, sans-serif', 'size': 12},

    'figure.titlesize': BASLIK_FONT_BOYUTU,

    'axes.titlesize': BASLIK_FONT_BOYUTU,        'title': {'font': {'size': 20}},

    'axes.labelsize': ETIKET_FONT_BOYUTU,

    'xtick.labelsize': ETIKET_FONT_BOYUTU,# Font ayarları        'plot_bgcolor': 'white',

    'ytick.labelsize': ETIKET_FONT_BOYUTU,

    'legend.fontsize': LEJANT_FONT_BOYUTU,BASLIK_FONT_BOYUTU = 14        'paper_bgcolor': 'white',

    'legend.framealpha': 0.8,

    'legend.edgecolor': '#DEE2E6'ETIKET_FONT_BOYUTU = 10        'margin': {'t': 100, 'b': 80, 'l': 80, 'r': 80},
...
def format_etiket(deger: float, toplam: float, min_yuzde: float = 3.0) -> str:
    ...
    """Pasta grafik dilimlerindeki etiketleri akıllı şekilde formatlar.        'legend': {

    

    Args:# Plotly tema ayarları            'orientation': 'h',

        deger: Dilimin değeri

        toplam: Toplam değerPLOTLY_TEMA = {            'yanchor': 'bottom',
    ...
    Returns:
        ...
        str: Formatlanmış etiket        'font': {'family': 'Arial, sans-serif', 'size': 12},            'xanchor': 'center',

    """

    yuzde = (deger / toplam) * 100        'title': {'font': {'size': 20}},            'x': 0.5

    

    if yuzde < min_yuzde:        'plot_bgcolor': 'white',        },
...
def renk_paleti_uret(n: int) -> List[str]:
    ...
    """İstenilen sayıda renk üretir.        'showlegend': True,}urma için sabit değerler ve yardımcı fonksiyonlar."""
    ...
    Returns:
        ...
        List[str]: Renk kodları listesi            'yanchor': 'bottom',

    """

    return GRAFIK_RENK_PALETI[:n] + GRAFIK_RENK_PALETI[:(n - len(GRAFIK_RENK_PALETI))]            'y': -0.2,# Erişilebilir ve kullanıcı dostu renk paleti (Color-blind friendly)



# Stil ayarlarını uygula            'xanchor': 'center',GRAFIK_RENK_PALETI = [

plt.style.use(GRAFIK_STIL)
