...
class GrafikOlusturucu:
    def tum_grafikleri_pdfde_birlestir(self, gun_tarihi: str, pdf_adi: str = None):
        ...
        # Grid ayarları (tek satırda yan yana)
        width_sum = sum(img.width for img in img_list)
        max_height = max(img.height for img in img_list)
        # Büyük bir boş görsel oluştur
        grid_img = Image.new("RGB", (width_sum, max_height), (255, 255, 255))
        x_offset = 0
        for img in img_list:
        ...
        pdf.add_page()
        pdf.image(str(grid_path), x=0, y=0, w=grid_img.width, h=grid_img.height)
        pdf.output(str(pdf_path))
        logger.info(f"Tüm grafikler tek PDF sayfasında birleştirildi: {pdf_path}")
        return pdf_path
