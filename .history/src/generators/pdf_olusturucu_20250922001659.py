...
class PDFOlusturucu:
    ...
    def pdf_olustur(self, grafik_dizini: Path, gun_tarihi: str) -> str:
        ...
        try:
            ...
            # Grafikleri 2x2 grid şeklinde yerleştir
            story.extend(self._grafikleri_grid_olarak_ekle(grafik_dosyalari, gun_tarihi))

            # PDF'i oluştur
            doc.build(story)
            logger.info(f"İçerik PDF'i oluşturuldu: {temp_pdf_dosyasi}")

            # Kapak sayfası ile birleştir
    ...
    def _grafikleri_grid_olarak_ekle(self, grafik_dosyalari: List[str], gun_tarihi: str) -> List:
        ...
        # Her 4 grafik için bir sayfa oluştur
        for i in range(0, len(grafik_dosyalari), 4):
            ...
            # 2x2 grid için tablo oluştur
            from reportlab.platypus import Table, TableStyle
            from reportlab.lib import colors

            data = []
            # Her grafik ve başlığı için bir satır
            for j in range(0, len(grafik_grubu), 2):
                row_items = []
                for k in range(j, j + 2):
                    if k < len(grafik_grubu):
                        grafik_path = grafik_grubu[k]
                        baslik_str = Path(grafik_path).stem.replace("_", " ").title()
                        ...
                        # Grafik ve başlığını içeren bir alt tablo
