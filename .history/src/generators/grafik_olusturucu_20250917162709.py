...
class GrafikOlusturucu:
    ...
    def pasta_grafik_olustur(self, veriler: pd.Series, baslik: str, dosya_adi: str):
        """Pasta grafiği veya bar grafiği oluşturur - hem sayı hem yüzde gösterir"""
        try:
            # Grafik ayarlarını uygula
            grafik_ayarlarini_uygula()
            ...
            if len(veriler) > 10:
                top_10 = veriler.nlargest(10)
                diger_toplam = veriler[~veriler.index.isin(top_10.index)].sum()
                if diger_toplam > 0:
                    top_10 = pd.concat([top_10, pd.Series({"Diğer": diger_toplam})])
                veriler = top_10
            ...
            if len(veriler) == 1:
                # Tek değer için bar grafik çiz
                plt.bar([0], veriler.values, color=renk_paleti_uret(1)[0])
                plt.xticks([0], [veriler.index[0]])
                plt.ylabel("Vaka Sayısı")
                total = veriler.sum()
                yuzde = 100.0
                plt.text(0, veriler.values[0], f"{int(veriler.values[0]):,}\n(%{yuzde:.1f})", 
                        ha="center", va="bottom", fontsize=10, fontweight="bold")
                plt.title(baslik, pad=20, fontsize=14, fontweight="bold")
            else:
                ...
                # Renk paleti oluştur
                renkler = renk_paleti_uret(len(veriler))
            ...
            # Save dosyasını oluştur
            if not os.path.exists(os.path.dirname(dosya_adi)):
                os.makedirs(os.path.dirname(dosya_adi), exist_ok=True)
            plt.savefig(dosya_adi, bbox_inches="tight", dpi=200)
            plt.close()
            return True
        except Exception as e:
            logging.error(f"Pasta grafiği oluşturma hatası: {str(e)} (dosya: {os.path.basename(dosya_adi)}, veri boyutu: {len(veriler)})")
            plt.close()
            return False
