...
class PDFOlusturucu:
    ...
    def _grafik_hucre_olustur(self, grafik_data: Tuple[Path, str]):
        ...
        try:
            ...
            mini_table = Table(cell_content, colWidths=[3.5*inch])
            return mini_table
    ...
    def _grafik_boyutu_hesapla(self, grafik_dosyasi: Path) -> Tuple[float, float]:
        ...
        try:
            ...
            # 4'lü grid için daha küçük boyutlar (2x2 düzen)
            max_genislik = 3.5 * inch  # Her grafik için yarı sayfa genişliği
            max_yukseklik = 2.8 * inch  # Daha kısa yükseklik
