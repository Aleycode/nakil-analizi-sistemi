...
def gunluk_nakil_analizi_yap(
    ...
    try:
        if gun_tarihi is None:
            ...
            # Kullanıcı tarih verdi, gun_tipi'ne göre +24 saat ekle
            if gun_tipi == "bugun":
...
def console_menu():
    ...
    # Menu utils'i import et
    from src.utils.menu_utils import excel_dosyasi_sec, menu_basligi_goster

    while True:
        menu_basligi_goster("🏥 NAKİL VERİ ANALİZ SİSTEMİ")
        print()
        ...
        try:
            ...
            if secim == "1":
                menu_basligi_goster("📥 GÜNLÜK VERİ İŞLEME")
                dosya_yolu = excel_dosyasi_sec()
                if dosya_yolu:
                    gunluk_islem_yap(dosya_yolu)
                
            elif secim == "2":
                menu_basligi_goster("📊 GÜNLÜK NAKİL ANALİZİ")
                print("\n1. Dün analizi (Dün 08:00 - Bugün 08:00)")
                print("2. Bugün analizi (Bugün 08:00 - Yarın 08:00)")
                print("3. Özel tarih analizi")
                ...
                if analiz_secim == "1":
                    ...
                    tarih = input("\n📅 Analiz tarihi (YYYY-MM-DD): ").strip()
                    gun_tipi = input("🔄 Analiz tipi (dun/bugun): ").strip().lower()
                    if tarih and gun_tipi in ["dun", "bugun"]:
                        gunluk_nakil_analizi_yap(tarih, gun_tipi)
                    else:
                        print("❌ Geçersiz tarih veya analiz tipi!")
                
            elif secim == "3":
                menu_basligi_goster("🔄 PARQUET → EXCEL DÖNÜŞTÜRME")
                print("\n⚠️ Bu özellik henüz geliştirme aşamasında...")
...
def main():
    """Ana fonksiyon"""
    # Eğer komut satırı argümanları varsa, onları işle
    if len(sys.argv) > 1:
