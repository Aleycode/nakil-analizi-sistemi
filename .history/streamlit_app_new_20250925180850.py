...
import sys
import os
from datetime import datetime, timedelt            try:
                if file_path.suffix.lower() == ".xls":
                    # .xls dosyaları için önce xlrd dene, başarısızsa pandas default engine'ini kullan
                    try:
                        df = pd.read_excel(file_path, engine="xlrd")
                    except Exception as xlrd_error:
                        st.warning(f"⚠️ xlrd ile okuma başarısız: {str(xlrd_error)[:100]}...")
                        st.info("🔄 Pandas default engine deneniyor...")
                        try:
                    ...
                    df = pd.read_excel(file_path, engine="openpyxl")rt subprocess
...
def show_graphs(date_folder, num_graphs=6):
    ...
    for i, graph in enumerate(display_graphs):
        with cols[i % 2]:
            st.image(str(graph), caption=graph.name, width="stretch")
    ...
    if not excel_files:
        ...
        if selected_file:
            ...
            try:
                ...
                st.info("💡 İpucu: .xls dosyaları için xlrd, .xlsx dosyaları için openpyxl kullanılıyor.")
                st.info("🔧 Eksik kütüphaneler varsa şu komutu çalıştırın: `pip install xlrd openpyxl`")
                
                # Hata detayları
                with st.expander("Hata Detayları"):
                    st.code(f"Dosya: {file_path}")
                    st.code(f"Uzantı: {file_path.suffix}")
                    st.code(f"Hata: {str(e)}")
...
def rapor_sayfasi():
    ...
    if st.session_state.get("selected_date"):
        st.info(f"🔍 Session'dan gelen tarih: {st.session_state.selected_date}")
    
    # Görüntülenecek rapor seçimi
    # Eğer session_state'den gelen bir tarih varsa onu varsayılan yap
    default_index = 0
    if st.session_state.get("selected_date") and st.session_state.selected_date in filtered_dates:
        default_index = filtered_dates.index(st.session_state.selected_date)
        st.success(f"✅ Otomatik seçim: {st.session_state.selected_date}")
    
    selected_date = st.selectbox("Görüntülenecek raporu seçin:", filtered_dates, index=default_index)
        ...
        if report_folder.exists():
            ...
            if png_files:
                ...
                with st.expander("Son rapor örneği"):
                    st.image(str(sample_graph), caption=f"{latest_date} - {sample_graph.name}", width="stretch")
            
            # Rapora git butonu
            if st.button("Son Raporu Görüntüle"):
                st.session_state.page = "rapor"
                st.session_state.selected_date = latest_date
                st.success(f"Rapor sayfasına yönlendiriliyor: {latest_date}")
                st.rerun()
        else:
...
def main():
    ...
    with st.sidebar:
        ...
        # Mevcut sayfa varsa onu varsayılan yap
        current_page = st.session_state.get("page", "ana_sayfa")
        if current_page not in menu_options:
            current_page = "ana_sayfa"
        
        # Mevcut sayfayı index olarak bul
        current_index = list(menu_options.keys()).index(current_page)
        
        selected_page = st.radio("Menü:", list(menu_options.values()), index=current_index)
