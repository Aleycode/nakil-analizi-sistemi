...
def gunluk_nakil_analizi_yap(
    ...
    try:
        ...
        gunluk_dizin_str = (gun_datetime - timedelta(days=1)).strftime("%Y%m%d") if gun_tipi == "dun" else gun_datetime.strftime("%Y%m%d")
        
        # Düzeltme: Bugün analizi için dünün verisi kullanılmalı
        if gun_tipi == "bugun":
             gunluk_dizin_str = (gun_datetime - timedelta(days=1)).strftime("%Y%m%d")
