...
def process_daily_data(file_path):
    """Günlük veri işleme - Streamlit Cloud uyumlu basit versiyon"""
    try:
        import pandas as pd
        from pathlib import Path
        ...
        if file_path.suffix.lower() == '.xls':
            # .xls dosyaları için xlrd engine kullan
            df = pd.read_excel(file_path, engine='xlrd')
        elif file_path.suffix.lower() == '.xlsx':
            # .xlsx dosyaları için openpyxl engine kullan  
            df = pd.read_excel(file_path, engine='openpyxl')
        else:
            raise ValueError("Desteklenen format değil. .xls veya .xlsx dosyası yükleyin.")
...
📋 {df.shape[1]} sütun bulundu
📅 Analiz için hazır!

💡 Şimdi Nakil Analizi sayfasına gidebilirsiniz"""
                self.stderr = ""
        
        return SuccessResult(len(df))
